  ...
  try {
    ...
    if (contract.expiresAt) {
      ...
      if (new Date() > expiresDate && contract.status !== "signed" && contract.status !== "completed") {
        await updateDocData("contracts", contract.id, { status: "expired" });
    ...
    await updateDocData("contracts", contract.id, updateData);
    ...
    const publicContract = {
      ...
      contractDate: contract.contractDate?.toDate ? contract.contractDate.toDate() : contract.contractDate,
      startDate: contract.startDate?.toDate ? contract.startDate.toDate() : contract.startDate,
      endDate: contract.endDate?.toDate ? contract.endDate.toDate() : contract.endDate,
      renderedHtml: contract.renderedHtml,
      status: updateData.status || contract.status,
      expiresAt: contract.expiresAt?.toDate ? contract.expiresAt.toDate() : contract.expiresAt,
...
export async function PATCH(request, context) {
  try {
    ...
    await updateDocData("contracts", contract.id, { analytics });
